...
LuminaraQuiz.prototype.loadGauntletQuestions = async function(difficulty) {
  // Use QuizModes' loaded questions if available
  if (typeof QuizModes !== 'undefined' && QuizModes.questions.length > 0) {
    this.gauntletQuestions = [...QuizModes.questions];
    QuizModes.shuffleArray(this.gauntletQuestions);
  } else {
    ...
    for (const cat of this.registry.categories) {
      for (const bank of cat.banks) {
        try {
          ...
          if (data.questions) {
            this.gauntletQuestions.push(...data.questions);
          }
        } catch (e) {}
      }
    }
...
LuminaraQuiz.prototype.renderGauntletQuestion = function() {
  ...
  arena.innerHTML = `
    <div class="question-card gauntlet-question">
      ...
      <div class="q-text">${this.renderer.escapeHtml(q.q)}</div>
      <div class="options">
        ${q.options.map((opt, i) => {
          ...
          return `<button class="${classes}" onclick="quiz.answerGauntlet(${i})">${this.renderer.escapeHtml(opt)}</button>`;
        }).join('')}
      </div>
    </div>
  `;
...
LuminaraQuiz.prototype.answerGauntlet = function(optIdx) {
  ...
  if (isCorrect) {
    ...
    // Wrong answer - take damage (use scaffold system)
    if (typeof scaffoldRemediation !== 'undefined') {
      scaffoldRemediation.startScaffoldSession(q.id, q._category || 'general');
    }

    // Still allow re-trying in Gauntlet (no mandatory scaffolds for speed)
  }
};
...
// Hook into mode switching for Gauntlet
